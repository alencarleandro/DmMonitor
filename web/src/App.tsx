...
import { api, localDate, sampleMeasurements, today } from './api'
...
function MeasurementDialog({ demo, onClose, onSave, notify }: { demo: boolean; onClose: () => void; onSave: (measurement: Measurement) => void; notify: (toast: Toast) => void }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      const measurement = demo ? { ...body, id: crypto.randomUUID(), measuredAt: new Date().toISOString(), context: 'other', notes: '' } : await api<Measurement>('/measurements', { method: 'POST', body: JSON.stringify(body) })
